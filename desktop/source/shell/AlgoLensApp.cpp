...
static std::string findBunExe() {
    // 0. Bundled next to the app (installer builds).
    {
        fs::path p = fs::path(exeDir()) / "bun.exe";
...
static std::string findServerDir() {
    // Installed layout: <exe>\server
    fs::path installed = fs::path(exeDir()) / "server";
    if (fs::exists(installed)) return installed.string();
    // Dev layout: desktop\build\<cfg> -> build -> desktop -> Algolens\server
    fs::path dev = fs::path(exeDir()) / ".." / ".." / ".." / "server";
...
static std::string findNodeExe() {
    // 0. Bundled next to the app (installer builds).
    {
        fs::path p = fs::path(exeDir()) / "node.exe";
        if (fs::exists(p)) return p.string();
    }
...
static std::string findTerminalDir() {
    // Installed layout: <exe>\terminal
    fs::path installed = fs::path(exeDir()) / "terminal";
    if (fs::exists(installed)) return installed.string();
    // Dev layout: desktop\build\<cfg> -> build -> desktop -> Algolens\terminal
    fs::path dev = fs::path(exeDir()) / ".." / ".." / ".." / "terminal";
    if (fs::exists(dev)) return fs::canonical(dev).string();
    return "";
}
...
void AlgoLensApp::OnContextInitialized() {
    ...
    if (serverDir.empty()) {
        algolens::ReportError("Execution server not found",
                              "Could not locate the server folder.");
    ...
    if (termDir.empty()) {
        algolens::ReportError("Terminal server not found",
                              "Could not locate the terminal folder.");
